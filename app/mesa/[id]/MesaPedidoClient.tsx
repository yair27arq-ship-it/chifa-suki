...
import { getMesaPorId } from '@/actions/platos';
...
import type { CategoriaMenu, Mesa, MetodoPago, PagoParte, Plato } from '@/types';
...
interface Props {
  ...
  initialPlatos: Plato[];
  initialMesa: Awaited<ReturnType<typeof getMesaPorId>>;
  initialPedido: Awaited<ReturnType<typeof getPedidoAbiertoDeMesa>>;
}

export function MesaPedidoClient({ mesaId, initialCategorias, initialPlatos, initialMesa, initialPedido }: Props) {
  const router = useRouter();

  const [mesa] = useState<Mesa | null>(initialMesa as Mesa | null);
  ...
  const { items, total, isPending, pedidoId, agregar, quitar, actualizarItem, cobrar, anular, notasItem } =
    usePedido('mesa', mesaId, null);
  ...
  return (
    <>
      ...
      <div className="content-top-row">
        <button onClick={() => router.push('/mesas')} className="back-btn" aria-label="Volver">
          <ChevronLeft size={22} />
        </button>
        <h1 className="page-title">{mesa?.nombre || `Mesa ${mesa?.numero}`}</h1>
      ...
      {showConfirmAnular && (
        <div className="confirm-backdrop">
          <div className="confirm-box">
            <h2 className="confirm-title">¿Anular pedido?</h2>
